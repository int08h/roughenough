...
pub enum ReportingError {
    #[error("{0}")]
    SerializationError(#[from] protocol::error::Error),

    #[error("remote server: {0}")]
    HttpError(String),
}

/// A [`Request`](protocol::request::Request)/[`Response`](protocol::response::Response) observation in a [`MalfeasanceReport`].
...
/// Ordered [`Request`](protocol::request::Request)/[`Response`](protocol::response::Response) observations that demonstrate a violation of causality.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn create_test_measurement(midpoint: u64, radius: u32) -> Measurement {
        let request = {
            let mut raw = include_bytes!("../../protocol/testdata/rfc-request.071039e5").to_vec();
            let mut cursor = ParseCursor::new(&mut raw);
            Request::from_frame(&mut cursor).unwrap()
        };

        let mut response = {
            let mut raw = include_bytes!("../../protocol/testdata/rfc-response.071039e5").to_vec();
            let mut cursor = ParseCursor::new(&mut raw);
