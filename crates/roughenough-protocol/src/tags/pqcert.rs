...
use crate::tag::Tag;
use crate::tags::pqsig::PQSignature;
...
impl Certificate {
    ...
    const QSIG_OFFSET: u32 = Self::DELE_OFFSET + size_of::<Delegation>() as u32;
    const OFFSETS: [u32; 2] = [Self::DELE_OFFSET, Self::QSIG_OFFSET];
    const TAGS: [Tag; 3] = [Tag::SIG, Tag::DELE, Tag::QSIG];

    pub fn new(sig: Signature, dele: Delegation, pqsig: PQSignature) -> Self {
        Self {
            signature: sig,
            delegation: dele,
            pq_signature: pqsig,
...
impl FromWire for Certificate {
    fn from_wire(cursor: &mut ParseCursor) -> Result<Self, Error> {
        let cert = Certificate {
            ...
            pq_signature: PQSignature::from_wire(cursor)?,
        };
...
impl ToWire for Certificate {
    ...
    fn to_wire(&self, cursor: &mut ParseCursor) -> Result<(), Error> {
        ...
        self.pq_signature.to_wire(cursor)?;
        Ok(())
    }
}
...
impl Debug for Certificate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CERT")
            ...
            .field("pq_signature", &self.pq_signature)
            .finish()
    }
}
