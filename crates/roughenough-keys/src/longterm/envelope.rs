...
pub struct SecretEnvelope {
    /// Identifier of the KMS key used to encrypt the secret
    pub key_id: String,

    /// Secret encrypted by the data encryption key (DEK)
    #[serde(with = "base64")]
...
pub(crate) fn seal_secret(dek: [u8; 32], secret: &Secret, aad: &[u8]) -> Vec<u8> {
    ...
    let mut in_out = secret.expose().to_vec();
    seal_key
...
pub(crate) fn open_secret(
    ...
    let mut in_out = encrypted_secret.to_vec();
...
#[cfg(test)]
mod tests {
    use crate::longterm::envelope::{open_secret, seal_secret};
    use crate::seed::Secret;
    ...
    fn seal_open_secret_roundtrip() {
        ...
        let original_secret = Secret::new(&original_secret_data);
        ...
        // Seal the secret
        let encrypted_secret = seal_secret(dek, &original_secret, aad);

        // Verify that the encrypted data is different from the original
        assert_ne!(encrypted_secret, original_secret_data);

        // Verify that the encrypted data is longer (due to authentication tag)
        assert!(encrypted_secret.len() > original_secret_data.len());

        // Open (decrypt) the secret
        let decrypted_secret =
            open_secret(dek, &encrypted_secret, aad).expect("Failed to decrypt secret");

        // Verify that the decrypted secret matches the original
        assert_eq!(decrypted_secret.expose(), original_secret.expose());
    }

    #[test]
    fn seal_open_secret_wrong_aad_fails() {
        ...
        let encrypted_secret = seal_secret(dek, &original_secret, correct_aad);

        // Attempting to decrypt with wrong AAD should fail
        let result = open_secret(dek, &encrypted_secret, wrong_aad);
        assert!(result.is_err(), "Decryption should fail with wrong AAD");
    }

    #[test]
    fn seal_open_secret_corrupted_data_fails() {
        // Test that decryption fails with corrupted encrypted data
        let original_secret = Secret::new_random();
        let dek = [0u8; 32];
        let aad = b"test_aad";

        let mut encrypted_secret = seal_secret(dek, &original_secret, aad);

        // Corrupt the encrypted data
        if let Some(byte) = encrypted_secret.get_mut(0) {
            *byte = byte.wrapping_add(1);
        }

        // Attempting to decrypt corrupted data should fail
        let result = open_secret(dek, &encrypted_secret, aad);
        assert!(
            result.is_err(),
