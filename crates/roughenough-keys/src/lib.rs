...
mod tests {
    ...
    fn generate_ltk() -> LongTermIdentity {
        ...
        LongTermIdentity::new(ProtocolVersion::DRAFT_14, Box::new(seed))
    }
    ...
    fn cert_is_created_correctly() {
        ...
        let mut to_verify = ProtocolVersion::DRAFT_14.dele_prefix().to_vec();
    ...
    fn online_key_generates_valid_srep_values() {
        ...
        let expected_vers = SupportedVersions::from([ProtocolVersion::DRAFT_14].as_ref());
        assert_eq!(srep.vers(), &expected_vers);

        let verifier = Verifier::from(&olk.public_key());
        let mut to_verify = ProtocolVersion::DRAFT_14.srep_prefix().to_vec();
