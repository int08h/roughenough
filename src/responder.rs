...
impl Responder {
    pub fn new(version: Version, config: &dyn ServerConfig, ltk: &mut LongTermKey) -> Responder {
        ...
        let requests = Vec::with_capacity(128);
    ...
    /// Send responses to all queued requests
    ...
    fn make_response(&self, srep: &RtMessage, cert_bytes: &[u8], path: &[u8], idx: u32) -> RtMessage {
        ...
        let mut response = RtMessage::new(6);
        response.add_field(Tag::SIG, sig_bytes).unwrap();

        if self.version == Version::Rfc {
            response.add_field(Tag::VER, &[0x01, 0x00, 0x00, 0x00]).unwrap();
        }
